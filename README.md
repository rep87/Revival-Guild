...
## 퀘스트 타임라인 & 캠프
- 일반 등급 퀘스트는 5~6턴, S급 퀘스트는 8~12턴으로 길이가 재조정되어 장기 탐험의 리듬이 살아납니다.
...
- 신규 퀘스트는 `QUEST_CONFIG.spawnRate`(현재 60%) 확률로 등장하며, 등급별 보상은 `rewardMultiplier` 테이블을 곱해 산출됩니다.
...
- 도감에는 용병을 처음 생성하거나 화면에 표시할 때 자동 등록되며, 퀘스트 완료 시 완료 횟수와 최근 본 턴이 갱신됩니다.
...
- CONFIG 덤프에는 `QUEST_CONFIG.spawnRate`와 등급별 `rewardMultiplier`도 함께 출력되어 스폰/보상 밸런스 점검이 쉬워집니다.
...
## 저장 및 초기화
- 모든 진행 상황은 `localStorage`의 `rg_v1_save` 키에 저장되며, `meta.saveVersion = 3` 필드로 버전 호환성을 관리합니다. 이전 버전 세이브는 로드시 자동으로 보정·상향 저장됩니다.
